...
```shell
gem install ./digital_femsa-1.0.2.gem
```

(for development, run `gem install --dev ./digital_femsa-1.0.2.gem` to install the development dependencies)

or publish the gem to a gem hosting service, e.g. [RubyGems](https://rubygems.org/).

Finally add this to the Gemfile:

    gem 'digital_femsa', '~> 1.0.2'
...
- **Update version**
  - Usage: `make update-version VERSION=1.0.1`
  - This command updates the following locations to the provided version:
    - `VERSION`
    - `lib/digital_femsa/version.rb` (constant `VERSION`)
    - `config-ruby.json` (field `gemVersion`)
    - `README.md` references (package version line, gem filename and Gemfile snippet)
